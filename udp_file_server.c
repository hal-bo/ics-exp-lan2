...
main(int argc, char** argv)
{
    ...
    for(i=2;i<argc;i++) {
      ...
      err = getaddrinfo(argv[i], port_num_str, &hints, &res[i-2]);
      if (err != 0) {
          perror("getaddrinfo");
          return 1;
      }
      addr.s_addr = ((struct sockaddr_in *)(res[i-2]->ai_addr))->sin_addr.s_addr;
      printf("ip address: %s\n", inet_ntoa(addr));
      printf("port#: %d\n", ntohs(((struct sockaddr_in*)(res[i-2]->ai_addr))->sin_port));
    }
    ...
    for( ; ; ) {                            /* 無限ループ */
        ...
        for(i=2;i<argc;i++) {
            ...
            while((n = read(fd, buf, BUF_LEN)) > 0) {
                printf("*");
                //printf("send: %s\n", buf);
                if(sendto(sock, buf, n, 0,res[i-2]->ai_addr, res[i-2]->ai_addrlen) /* ADD HERE */ != n) {
                    perror("sendto");
                    break;
                }
                // if(sendto(sock, buf, n, 0,res[1]->ai_addr, res[1]->ai_addrlen) /* ADD HERE */ != n) {
                //     perror("sendto");
                //     break;
                // }
                // if(sendto(sock, "quit", 4, 0,res[1]->ai_addr, res[1]->ai_addrlen) /* ADD HERE */ != 4) {
                //     perror("sendto");
                //     break;
                // }
            }
            if(sendto(sock, "quit", 4, 0,res[i-2]->ai_addr, res[i-2]->ai_addrlen) /* ADD HERE */ != 4) {
                perror("sendto");
                break;
            }
            printf("\nsend %d finish\n");
